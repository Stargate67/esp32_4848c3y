...
void gt911_lvgl_touch_cb(lv_indev_t *indev, lv_indev_data_t *data)
{
    ...
    if (pressed && touch_cnt > 0)
    {
        ...
        log_v(" ********  FY   Pressed at: (%d,%d)", data->point.x, data->point.y);
    }
    else
        data->state = LV_INDEV_STATE_RELEASED;

    log_v(" ********  FY    Touch handle Call back ");
}
