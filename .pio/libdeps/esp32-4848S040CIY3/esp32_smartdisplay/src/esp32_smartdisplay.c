...
void lvgl_touch_calibration_transform(lv_indev_t *indev, lv_indev_data_t *data)
{
  ...
  if (touch_calibration_data.valid && data->state == LV_INDEV_STATE_PRESSED)
  {
    ...
    log_d("  ***********   FY Calibrate point (%d, %d) => (%d, %d)", data->point.x, data->point.y, pt.x, pt.y);
...
touch_calibration_data_t smartdisplay_compute_touch_calibration(const lv_point_t screen[3], const lv_point_t touch[3])
{
  ...
  log_d("AFTER CALIBRATION alphaX: %f, betaX: %f, deltaX: %f, alphaY: %f, betaY: %f, deltaY: %f", touch_calibration_data.alphaX, touch_calibration_data.betaX, touch_calibration_data.deltaX, touch_calibration_data.alphaY, touch_calibration_data.betaY, touch_calibration_data.deltaY);
